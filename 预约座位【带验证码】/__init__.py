...
def get_userid_from_txt(libiary_username, libiary_password):  # 获取user_id（取代 user_id = "181919"  # 使用者id 宝顺才：185469 李国兴：181919）
...
if __name__ == "__main__":
    ...
    with open('information.txt', 'r', encoding='utf-8') as file:
        ...
        # 读取并赋值给变量
        tujianocr_uname = lines[0].split('：', 1)[-1].strip()
        # print("图鉴识别账号:", tujianocr_uname)
        tujianocr_pwd = lines[1].split('：', 1)[-1].strip()
        # print("图鉴识别密码:", tujianocr_pwd)
    ...
    userid = get_userid_from_txt(libiary_username=libiary_username, libiary_password=libiary_password)
